...
class LocalPlanner(Node):
    def __init__(self):
        ...
        self.declare_parameter("moving_straight_kp", 3)
        self.declare_parameter("moving_straight_kd", 0)
        self.declare_parameter("moving_straight_ki", 0)
        ...
        self.declare_parameter("turning_kp", 5)
        self.declare_parameter("turning_kd", 5)
        self.declare_parameter("turning_ki", 0)
        ...
        self.moving_straight_kp = self.get_parameter(
            "moving_straight_kp").get_parameter_value().integer_value
        self.moving_straight_kd = self.get_parameter(
            "moving_straight_kd").get_parameter_value().integer_value
        self.moving_straight_ki = self.get_parameter(
            "moving_straight_ki").get_parameter_value().integer_value
    ...
    def determine_local_controller_strategy(self):
        if len(self.latestGlobalPath) < 2:
    ...
    def publish_left_and_right_pwm(self):
        left_pwm_input: int = self.current_local_planner_controller.left_pwm()
        right_pwm_input: int = self.current_local_planner_controller.right_pwm()
        left_pwm = UInt32()
        right_pwm = UInt32()
        ...
        self.get_logger().info("left pwm {0} right pwm {1}".format(
            self.left_servo_pwm, self.right_servo_pwm))
